    ...
    <div style="margin-top: 10px">
      <el-card>
        <el-row :gutter="10">
          <!--          分类-->
          <el-row :span="4">
            <div style="border: 1px dashed #ccc; text-align: center; border-bottom: none" class="radio">
              <el-radio-group size="medium"  class="category-item" @click.native="getGoodsByCategory(item.id)" v-for="item in categories" :key="item.id"
                   :class="{ active: (activeIndex === item.id)}"><el-radio-button  label="item.name" class="radioitem" >{{item.name }}</el-radio-button>
              </el-radio-group>
          ...
          <el-col :span="20">
            <!--          分类商品-->
            <div style="min-height: 150px" class="category">
              <el-row :gutter="10">
                <el-col :span="6" v-for="item in tableData" :key="item.id">
                  <div style="margin-bottom: 10px; margin-outside:10px;cursor: pointer"  @click="goodsDetail(item.id)">
...
.radio ::v-deep .el-radio-button__inner{
...
.radioitem ::v-deep .el-radio-button__inner{
  width: 210px;
  text-align: center;
}
.category{
